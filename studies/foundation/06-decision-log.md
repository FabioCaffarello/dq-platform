...
| B2-28 | Engine Dockerfile | open | Ship `engine/Dockerfile` satisfying ADR-0042 Clause 1 invariants (multi-stage build; non-root + readOnlyRootFilesystem + dropped caps + RuntimeDefault seccomp; image name `dq-engine:<tag>` derived mechanically from `engine-v<tag>` git tag). Closes the `dq-engine:placeholder` forward reference from W3-P7b. | ADR-0042 §"Clause 1 — Docker invariants" + Consequence #7 — contract is binding; specific base-image choice and build-cache layout are per-workspace decisions resolved by the slice. Flagged when ADR-0042 was promoted (B2-3 closure). | `engine/Dockerfile` + CI build-and-push workflow + base-image choice |
