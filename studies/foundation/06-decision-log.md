...
- Last updated: 2026-05-26 (B2-6 → ADR-0039: dashboard contract — two consumer-surface contracts committed (SQL tables + Prometheus-compatible `/metrics` endpoint); metric inventory formalized from foundation 05 with cardinality/evolution rules; baseline-dashboard implementation deferred as new B2-24 slice paced post-Phase-4c metric emission; three OQs reserved out-of-scope — `dq_entity_rollup` pre-agg, per-metric cardinality ceiling, documented field-level inventory for `evidence_summary` and `sample_violating_rows`)
