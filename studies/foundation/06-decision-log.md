...
| B2-27 | Window-endpoint columns implementation slice | open | Implement the additive `window_start` / `window_end` columns on `dq_executions` committed by ADR-0041 at the contract level. Adds `WindowStart` / `WindowEnd` to `engine/internal/results/types.go`'s `ExecutionRow`, ships the DDL migration, updates the `dq_executions_current` view projection, and amends ADR-0039's stable-column inventory. | ADR-0041 §"Window-endpoint columns (design-only)" — contract is binding now; implementation deferred to a slice paced post-Phase-4c metric emission so the new columns smoke-test end-to-end against actual metric output. Flagged when ADR-0041 was promoted (B2-4 closure). | `engine/internal/results/types.go` extension + DDL migration + view update + ADR-0039 inventory amendment |
