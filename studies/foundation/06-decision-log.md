...
| B2-32 | External-artifact-reference implementation slice | open | Implement the bounded external-reference contract from ADR-0044: extend catalog v1 with `external_eligible_fields` and add `schema_ref` sibling to `record.schema_conformance`'s `params_schema`; extend `dq-manifest publish` to walk check params, run the three-step path-safety check (no-`..`, symlink canonicalization, rules-tree containment), read + parse + validate + inline at publish time; extend `dq-lint` with the same resolution plus lint cross-check #12 (mutual-exclusion of `<field>` and `<field>_ref`); ship a shared `pathsafe` helper; cover nine documented test fixtures (valid `_ref`, `..` rejection, symlink-escape rejection, non-eligible-field rejection, both-present rejection, missing-file, malformed JSON/YAML, validation failure, valid pass-through). | ADR-0044 §"Clause 6 — Implementation deferred" — contract is binding now; implementation is mechanical when the slice lands. No rule-schema bump needed (additive within catalog v1). Flagged when ADR-0044 was promoted (B2-1 closure). | Catalog v1 extension + canonical+mirror update + `dq-manifest publish` resolution + `dq-lint` cross-check #12 + shared `pathsafe` helper + nine test fixtures |
