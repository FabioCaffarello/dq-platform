...
| B2-30 | Deeper deploy-validation lane integrated into `validate-deploy` | open | Extend `make validate-deploy` with a deeper-validation lane (e.g., `kubeconform`, kind-based cluster, or equivalent) that catches field-name typos, deprecated API versions, and schema compliance against the Kubernetes API surface. Specific tool decided by the slice. Closes W3-P7c's deferral + ADR-0042 Clause 4. | ADR-0042 §"Clause 4 — Deploy-manifest deeper-validation posture" — contract binds the follow-up to extend the existing target, not introduce a new top-level CI lane. Flagged when ADR-0042 was promoted (B2-3 closure). | Makefile extension + tool integration |
