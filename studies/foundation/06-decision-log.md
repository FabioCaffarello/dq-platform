...
- Last updated: 2026-05-26 (B2-27 implementation shipped against ADR-0041 — `window_start` / `window_end` columns added to `dq_executions` (Required=true; pre-B2-27 deployments need ALTER TABLE backfill); `ExecutionRow` struct + BigQuery `executionRecord` round-trip both; `runningOlderThanSQL` + `currentExecutionsInlineSQL` SELECT lists extended; three runner.go call sites + orphan's followup-row constructor populate window endpoints; ADR-0039's stable-column inventory amended; full engine test suite + per-module sweep clean; paced ahead of Phase-4c metric emission per operator request)
