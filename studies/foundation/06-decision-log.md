...
| B2-31 | `DQ_LOG_LEVELS` per-package override implementation | open | Implement the per-package logging-override mechanism committed by ADR-0043: parse `DQ_LOG_LEVELS` into a new `EnvConfig.LogLevels map[string]slog.Level` field; wire a custom slog handler that resolves a `component` attribute on each log call against the map via longest-prefix-match at dot boundaries; adopt `slog.With("component", "engine.<name>")` as the per-package instantiation convention across the engine's nine `internal/` packages; ship the startup-time audit log line for ignored-package entries; cover grammar (case-insensitive, whitespace-trim), precedence, error handling, defaults in unit tests. | ADR-0043 §"Implementation posture (deferred)" — contract is binding now; implementation is a single mechanical slice. Flagged when ADR-0043 was promoted (B2-2 closure). | `EnvConfig.LogLevels` field + custom slog handler + per-package `slog.With` adoption + unit tests + startup audit log line |
