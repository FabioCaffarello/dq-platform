...
| B2-29 | Per-workspace tag-prefix CI gate | open | Ship the CI lane that rejects a push of `<workspace>-v*` whose diff-from-prior-tag touches files outside `<workspace>/` (e.g., `engine-v*` touching `rules/`). Closes W2-5 §C-W2-5.5 + ADR-0042 Clause 3. | ADR-0042 §"Clause 3 — Versioning invariants" — contract is binding; CI implementation deferred. Flagged when ADR-0042 was promoted (B2-3 closure). | New CI workflow under `.github/workflows/` consuming `git diff <prior-tag>...HEAD` against the tag's workspace scope |
